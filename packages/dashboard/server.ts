...
export function createServer(options: ServerOptions = {}) {
  ...
  app.put('/api/locales/:file', (req, res) => {
    try {
      ...
      res.status(500).json({
        error: 'Failed to update locale file',
        message: (error as Error).message,
      });
    }
  });
  ...
  app.post('/api/locales/add/key', (req, res) => {
    try {
      const { key, value, force } = req.body;

      if (key === '' || value === '') {
        return res.status(400).json({ error: 'Key or value is not provide' });
      }

      addToFiles(LOCALES_DIR, key, value, force);

      return res.json({
        message: 'Keys has been added to all files',
      });
    } catch (error) {
